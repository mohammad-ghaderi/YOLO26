    ...
    for (int ih = 0; ih < SIZE; ih++) {
        for (int iw = 0; iw < SIZE; iw+=mr) {
            for (int oc = 0; oc < OC; oc+=nr) {
                point_wise_5x16(inp+(ih*SIZE+iw)*IC, pack_w+oc*IC, output+(ih*SIZE+iw)*OC+oc, IC, OC, (OC-nr)*4);
            }
        }
    }
}
...
void pointwise_conv5x20(float *inp, float *weights, float *output, int IC, int OC, int SIZE) {
    ...
    for (int ih = 0; ih < SIZE; ih++) {
        for (int iw = 0; iw < SIZE; iw+=mr) {
            for (int oc = 0; oc < OC; oc+=nr) {
                point_wise_5x20(inp+(ih*SIZE+iw)*IC, pack_w+oc*IC, output+(ih*SIZE+iw)*OC+oc, IC, OC, (OC-nr)*4);
            }
        }
    }
}
